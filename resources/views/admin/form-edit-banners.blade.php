                ...
                <div class="col-md-12">
                  ...
                  <div class="card card-outline card-primary">
                    <div class="card-header">
                      <h3 class="card-title mb-0">Edição de Banners </h3>
                    </div>
                    <!-- /.card-header -->
                    <!-- form start -->
                    <form action="{{route('banners.update', $banner->id)}}" method="post" enctype="multipart/form-data">
                        ...
                        <input type="text" name="titulo" class="form-control" id="titulo" placeholder="Título" value="{{ $banner->titulo }}" required>
                        </div>
                        <div class="form-group col-sm-12">
                          <label for="descricao">Descrição</label>
                        <textarea name="descricao" class="form-control" id="descricao" placeholder="Descrição">{{ $banner->descricao }}</textarea>
                        </div>
                        <div class="form-group col-sm-12">
                            <div class="card card-outline card-primary">
                                <div class="card-header d-flex align-items-center">
                                  <h5 class="card-title mb-0">Selecione uma imagem &nbsp</h5>
                                  <small id="telefoneHelpBlock" class="form-text text-muted">
                                    (tamanho ideal do banner 1280x512px)
                                  </small>
                                </div>
                                <div class="card-body">
                                    <input type="file" name="image" />
                                    <img src="{{ URL::to('/') }}/image_banners/{{ $banner->banner }}" class="img-thumbnail" width="100" />
                                    <input type="hidden" name="hidden_image" value="{{ $banner->banner }}" />
                                </div>
                                <!-- /.card-body -->
                            </div>
                        </div>
